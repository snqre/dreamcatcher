...
contract SolsticeHost {
    ...
    struct Solstice {
        ...
        address[] recipientsEntryFees;
        address[] recipientsExitFees;
    }

    IQuickSwapPlugIn plugIn = IQuickSwapPlugIn(0x00F66EAC4d1e4e4c54ea2E289cF2FD0Ce04C3f78);
    IRepository repository = IRepository(0xE2578e92fB2Ba228b37eD2dFDb1F4444918b64Aa);
    ...
    function createNewVault(string memory name, string memory description) public returns (Solstice memory newSolstice) {
        uint newIndex = _increment();
        newSolstice.index = newIndex;
        newSolstice.name = name;
        newSolstice.description = description;
        newSolstice.denominator = IERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F);
        newSolstice.launch = block.timestamp;
        newSolstice.admins[0] = msg.sender;
        newSolstice.managers[0] = msg.sender;
        newSolstice.authorizedIn[0] = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
        newSolstice.authorizedIn[1] = 0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6;
        newSolstice.authorizedIn[2] = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
        newSolstice.authorizedIn[3] = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
        newSolstice.authorizedIn[4] = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
        newSolstice.authorizedIn[5] = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
        newSolstice.authorizedOut[1] = address(newSolstice.denominator);
        newSolstice.recipientsStreamingFees[0] = msg.sender;
        newSolstice.recipientsEntryFees[0] = msg.sender;
        newSolstice.recipientsExitFees[0] = msg.sender;
    }
    ...
    function _load(uint index) private view returns (Solstice memory solstice) {
        ...
        bytes memory encodedSolstice = repository.getBytes(addressSetSolsticeIndex);
        solstice = abi.decode(encodedSolstice, (Solstice));
        return solstice;
    }

    function _save(uint index, Solstice memory solstice) private {
        bytes32 addressSetSolsticeIndex = keccak256(abi.encode("solstice", index));
        repository.setBytes(addressSetSolsticeIndex, abi.encode(solstice));
    }
