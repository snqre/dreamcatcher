...
contract TerminalV2 is ProxyStateOwnableContract {
    ...
    */
    function deployed() public view returns (address[] memory) {
        return _addressSet[keccak256(abi.encode("proxies"))];
    }
    ...
    */
    function active() public view returns (address[] memory) {
        return _addressSet[keccak256(abi.encode("active"))];
    }
    ...
    */
    function update() public onlyOwner whenNotPaused() {
        if (_book[keccak256(abi.encode("updated"))]) { revert AlreadyReinitialized(); }
        _grant(owner());
    ...
    */
    function upgrade(address implementation) public override onlyAuthorized() whenNotPaused() {
        super.upgrade(implementation);
    ...
    */
    function upgradeBatchTo(address[] memory accounts, address implementations) public onlyAuthorized() whenNotPaused() {
    ...
    */
    function deploy() public onlyAuthorized() whenNotPaused() {
        address deployed = new ProxyStateOwnableContract();
    ...
    */
    function call(address account, string memory signature, bytes memory args) public onlyAuthorized() whenNotPaused() {
        _call(account, signature, args);
    }
    ...
    * @event PermissionGranted Emitted when an address is successfully granted authorization.
    */
    ...
    * @event PermissionRevoked Emitted when an address is successfully revoked of authorization.
    */
