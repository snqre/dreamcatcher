...
contract DreamToken is ERC20, ERC20Burnable, ERC20Snapshot, ERC20Permit {
    ...
    function balance_(address account)
    external pure
    returns (bytes32) {
...
/**
contract SolsticeTokenBeta is ERC20, ERC20Burnable, ERC20Snapshot, ERC20Permit {
...
contract SolsticeBeta {
    ...
    //SolsticeTokenBeta public token;

    // mapping for index to asset contract
    ...
    constructor(
        ...
        safeguard = ISolsticeSafeguardBeta(0xA2Da3E254333ACDF325D737D17AdE18EbE2bA7Cf);
        safeguard.setName(name);
        safeguard.setDescription(description);
        safeguard.setDenominator(denominator);
        safeguard.pushTokenContract(denominator);
        safeguard.pushTokenAmount(0);
        ...
        //token = new SolsticeTokenBeta(nameToken, symbolToken);
    }
    ...
    public {
        ...
        uint denominatorBalanceBefore = safeguard.getTokenAmount(0);
        asset.transferFrom(
            msg.sender, 
            address(this), 
            amount
        );
        if (address(asset) != safeguard.getDenominator()) {
            oracle.swapTokens(
                address(asset),
                safeguard.getDenominator(),
                amount,
                100,
                address(this)
            );
        }
        uint denominatorBalanceAfter = safeguard.getTokenAmount(0);
        uint amountReceived = denominatorBalanceAfter - denominatorBalanceBefore;
        require(
    ...
    function _amountToMint(uint value, uint supply, uint balance)
    private pure
    returns (uint amount) {
        /** non of these values must ever be zero */
        require(
            value        >= 1
            && supply    >= 1
            && balance   >= 1,
            "SolsticeBeta: cannot calculcate mint because of insufficient math"
        );
        return (value * supply) / balance;
    }

    function _amountToSend(uint amount, uint supply, uint balance)
    private pure
    returns (uint value) {
        /** non of these values must ever be zero */
        require(
            ...
            && supply    >= 1
            && balance   >= 1,
            "SolsticeBeta: cannot calculate send because of insufficient math"
        );
        return (amount * balance) / supply;
    }
}

contract SolsticeFactoryBeta {
    ISolsticeSafeguardBeta public safeguard;
    ...
    function deploySolsticeVault()
    public {
        uint count = safeguard.incrementImplementationCount();
        safeguard.addImplementation();
    }
}