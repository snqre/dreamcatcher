...
library SolsticeToolkit {
    function isAdmin(IRepository repository, uint index, address account)
    external view
    returns (bool) {
        bytes32 admins = keccak256(abi.encode("solstice", index, "admins"));
        return repository.addressSetContains(admins, account);
    }

    function isManager(IRepository repository, uint index, address account)
    external view
    returns (bool) {
        bytes32 managers = keccak256(abi.encode("solstice", index, "managers"));
        return repository.addressSetContains(managers, account);
    }

    function isContributor(IRepository repository, uint index, address account)
    external view
    returns (bool) {
        bytes32 contributors = keccak256(abi.encode("solstice", index, "contributors"));
        return repository.addressSetContains(contributors, account);
    }

    function getAdmins(IRepository repository, uint index)
    external view
    ...
    function getManagers(IRepository repository, uint index)
    external view
    returns (address[] memory) {
        bytes32 managers = keccak256(abi.encode("solstice", index, "managers"));
    ...
    function getContributors(IRepository repository, uint index)
    external view
    returns (address[] memory) {
        bytes32 contributors = keccak256(abi.encode("solstice", index, "contributors"));
    ...
    function getName(IRepository repository, uint index)
    external view
    returns (string memory) {
        bytes32 name = keccak256(abi.encode("solstice", index, "name"));
        return repository.getString(name);
    }

    function getDescription(IRepository repository, uint index)
    external view
    returns (string memory) {
        bytes32 description = keccak256(abi.encode("solstice", index, "description"));
        return repository.getString(description);
    }

    function getContribution(IRepository repository, uint index, address account)
    external view
    returns (uint) {
    ...
    function isWhitelisted(IRepository repository, uint index)
    external view
    returns (bool) {
        bytes32 whitelisted = keccak256(abi.encode("solstice", index, "whitelisted"));
        return repository.getBool(whitelisted);
    }

    function addAdmin(IRepository repository, uint index, address account)
    external {
        bytes32 admins = keccak256(abi.encode("solstice", index, "admins"));
        repository.addAddressSet(admins, account);
    }

    function addManager(IRepository repository, uint index, address account)
    external {
        bytes32 managers = keccak256(abi.encode("solstice", index, "managers"));
        repository.addAddressSet(managers, account);
    }

    function addContributor(IRepository repository, uint index, address account)
    external {
        bytes32 contributors = keccak256(abi.encode("solstice", index, "contributors"));
        repository.addAddressSet(contributors, account);
    }

    function removeAdmin(IRepository repository, uint index, address account)
    external {
        bytes32 admins = keccak256(abi.encode("solstice", index, "admins"));
        repository.removeAddressSet(admins, account);
    }

    function removeManager(IRepository repository, uint index, address account)
    external {
        bytes32 managers = keccak256(abi.encode("solstice", index, "managers"));
        repository.removeAddressSet(managers, account);
    }

    function removeContributor(IRepository repository, uint index, address account)
    external {
        bytes32 contributors = keccak256(abi.encode("solstice", index, "contributors"));
        repository.removeAddressSet(contributors, account);
    }

    function setName(IRepository repository, uint index, string memory newName)
    external {
        bytes32 name = keccak256(abi.encode("solstice", index, "name"));
        repository.setString(name, newName);
    }

    function setDescription(IRepository repository, uint index, string memory newDescription)
    external {
        bytes32 description = keccak256(abi.encode("solstice", index, "description"));
        repository.setString(description, newDescription);
    }

    function setContribution(IRepository repository, uint index, address account, uint newContribution)
    external {
        bytes32 contribution = keccak256(abi.encode("solstice", index, account, "contribution"));
