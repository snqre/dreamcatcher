...
class Client {
    ...
    public static async connect(): Promise<string[]> {
        ...
        if (!window) {
            console.error("Client::Connect::WindowNotFound");
            return [];
        }
        if (!(window as any).ethereum) {
            console.error("Client::Connect::ProviderNotInstalledOrRejected");
            return [];
        }
        Client._provider = new BrowserProvider((window as any).ethereum);
        try {
            let accounts: string[] = await Client._provider.send("eth_accounts", []);
            if (accounts.length > 0) {
                return accounts;
            }
            else {
                return await Client._provider.send("eth_requestAccounts", []);
            }
        }
        catch (error: unknown) {
...
class PurpleGradientButton {
    public static Component(props: RedToPinkGradientButtonProps): ReactNode {
        let {label, alias, spring, children, ... more} = props;
        const color0: string = ColorPalette.DEEP_PURPLE_GRADIENT.COLORS[0].toHex().toString();
        const color1: string = ColorPalette.DEEP_PURPLE_GRADIENT.COLORS[1].toHex().toString();
        const x0: string = "4px";
        const y0: string = "1px";
        const x1: string = "32px";
        const y1: string = "1px";
...
class Blurdot {
    public static Component(props: BlurdotProps): ReactNode {
        let {alias, color0, color1, spring, children, ... more} = props;
        alias = Controller.populateAlias(alias);
        return (
            <Col.Component
            alias={alias}
...
class NavbarButton {
    public static Component(props: NavbarButtonProps): ReactNode {
        const {to, text0, text1, style, disabled, children, ... more} = props;
        return (
            <Link
            /// @ts-expect-error
            style={{
            ...
            {... more}>
                ...
                style={{
                    "background": !disabled 
                        ? ColorPalette.DEEP_PURPLE.toHex().toString()
                        : ColorPalette.ROCK.toHex().toString(),
                    "fontSize": "1em",
                    "display": "flex",
                    "flexDirection": "row",
        ...
        return (
            ...
            spring={{
                ...
                style={{
                    ...
                    <Navbar._ConnectButton/>
                </Row.Component>
            </Row.Component>
        );
    }

    private static _ConnectButton(): ReactNode {
        return (
...
class HomePage {
    public static Component(): ReactNode {
        let [heroSectionFloatingState, setHeroSectionFloatingState] = useState<0 | 1 | 2>(2);
        useEffect(()=>{
            setTimeout(()=>{
                Controller.setSpringConfig("homePageHeroSectionMotion", {
                    velocity: 0.00000000000001,
                });
                Controller.setSpring("homePageHeroSectionMotion", {
                    "top": "0px",
                });
                setHeroSectionFloatingState(0);   
            }, 360 / 2);
        }, []);
        useEffect(()=>{
            setTimeout(()=>{
                const duration: number = 3600 * 4;
                Controller.setSpringConfig("homePageHeroSectionMotion", {
                    duration: duration,
                    easing: easings.easeInOutExpo,
                });
                if (heroSectionFloatingState === 0) {
                    Controller.setSpring("homePageHeroSectionMotion", {
                        top: "20px",
                    });
                    setTimeout(()=>{setHeroSectionFloatingState(1)}, duration);
                }
                if (heroSectionFloatingState === 1) {
                    Controller.setSpring("homePageHeroSectionMotion", {
                        top: "0px",
                    });
                    setTimeout(()=>{setHeroSectionFloatingState(0)}, duration);
                }
            }, 1050);
        }, [heroSectionFloatingState]);
        return (
            <Page.Component>
                ...
                <Layer.Component>
                    ...
                    style={{
                        ...
                        spring={{
                            ...
                            <Col.Component
                            alias="homePageHeroSectionMotion"
                            spring={{
                                "position": "relative",
                                "top": "1000px",
                            }}>
    ...
    private static _Footer(): ReactNode {
        return (
            <Col.Component
            spring={{
                marginTop: "60px",
                ...
                frameDir="to bottom">
                    ...
                    spring={{
                        ...
                        style={{
                            ...
                            <PurpleGradientButton.Component 
                            label="Get Started"/>
                        </Link>
...
class DiscoverPage {
    private static _searchInput: string = "";

    public static Component(): ReactNode {
        return (
            <Page.Component>
                <DiscoverPage._Background/>
                <Layer.Component>
                    <Navbar.Component/>
                    <Row.Component
                    style={{
    ...
    private static _SearchBar(): ReactNode {
        return (
            <Row.Component
            spring={{
                gap: "5px",
                ...
                style={{
                    ...
                    type="text"
                    spellCheck={false}
                    onChange={event=>DiscoverPage._searchInput = event.target.value}
                    ...
                    style={{
                        ...
                        outline: "none",
                        color: ColorPalette.POLISHED_TITANIUM.toHex().toString(),
                        fontSize: "0.8em",
                        background: "transparent",
                        pointerEvents: "fill",
                        cursor: "text",
                        ///borderColor: `${ColorPalette.GRAPHITE.toHex().toString()} 1`,
                        ///borderWidth: "1px",
                        ///borderStyle: "solid",
                        fontFamily: "Roboto Mono, monospace",
                        padding: "5px",
    ...
    private static _Background(): ReactNode {
        return (
            ...
            style={{
                ...
                <Blurdot.Component
                color0={ColorPalette.RELAXED_TEAL.toHex().toString()} 
                color1={ColorPalette.OBSIDIAN.toHex().toString()}
                spring={{
                    pointerEvents: "none",
                    width: "1000px",
...
class Renderable {
    public static render() {
        ...
        root.render(<RouterProvider router={createBrowserRouter([{
            ...
            path: "/discover",
            element: (<DiscoverPage.Component/>),
        }])}/>);
    }
}
